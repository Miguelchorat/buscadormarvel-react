...
const Listado_comic = () => {
    ...
    const obtenerDatos =  async () => {
        ...
        if(busqueda=='' ||busqueda==null){
            api = await fetch('https://gateway.marvel.com/v1/public/comics?limit='+DATOS_POR_PAGINA+'&offset='+indice+'&orderBy='+filtro+'&ts='+ts+'&apikey='+public_key+'&hash='+hash);
        }
        else{
            api = await fetch('https://gateway.marvel.com/v1/public/comics?limit='+DATOS_POR_PAGINA+'&offset='+indice+'&orderBy='+filtro+'&titleStartsWith='+busqueda+'&ts='+ts+'&apikey='+public_key+'&hash='+hash);
        }        
